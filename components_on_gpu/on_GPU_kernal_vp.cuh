...
__global__ void SupplySimulationPreVehiclePassing(GPUMemory* gpu_data, int time_step, int segment_length, GPUSharedParameter* data_setting_gpu, GPUVehicle *vpool_gpu) {
 	...
 	for (int i = 0; i < can_insert_veh; i++) {
 		...
 		gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[lane_index]
													+ (gpu_data->lane_pool.first_veh_index[lane_index]+gpu_data->lane_pool.vehicle_counts[lane_index])%gpu_data->lane_pool.ring_buffer_size[lane_index]]
 	...
 	for(int lane_index = gpu_data->seg_pool.lane_start_index[seg_index]; lane_index<=gpu_data->seg_pool.lane_end_index[seg_index]; ++lane_index){
 		...
 		for (; queue_index < gpu_data->lane_pool.vehicle_counts[lane_index]; queue_index++) {
 			int vehicle_index = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[lane_index]
																			+ (gpu_data->lane_pool.first_veh_index[lane_index]+queue_index)%gpu_data->lane_pool.ring_buffer_size[lane_index]];
...
__device__ void updateTimeDiffArray(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, int lane_id, int seg_id){
	...
	{
		int first_vehicle_ID = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[lane_id] + gpu_data->lane_pool.first_veh_index[lane_id]];
...
__device__ void getFirstVehicleID(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, int* veh_id, int* lane_index, int upstream_start_lane, int upstream_end_lane){
	...
	*lane_index = -1;
	...
	if(*lane_index>=0){
		*veh_id = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[*lane_index] + gpu_data->lane_pool.first_veh_index[*lane_index]];
	}
}
__device__ void updateAfterVehicleFinishingTrip(GPUMemory* gpu_data, GPUSharedParameter* data_setting_gpu, int lane_id, int seg_id){
	gpu_data->lane_pool.vehicle_counts[lane_id]--;
	gpu_data->lane_pool.first_veh_index[lane_id] = (gpu_data->lane_pool.first_veh_index[lane_id]+1)%gpu_data->lane_pool.ring_buffer_size[lane_id];
...
__device__ void updateAfterVehicleMovingToBuffer(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, GPUSharedParameter* data_setting_gpu, int time_step, int veh_id, int lane_id, int seg_id, int next_seg_id){
	...
	int new_vehicle_index = gpu_data->lane_pool.vehicle_start_index[next_lane] +
			(gpu_data->lane_pool.buffered_first_veh_index[next_lane] + gpu_data->lane_pool.buffered_vehicle_counts[next_lane])%(gpu_data->lane_pool.ring_buffer_size[next_lane]);
...
__device__ void updateAfterVehicleMovingToNextSeg(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, GPUSharedParameter* data_setting_gpu, int time_step, int veh_id, int lane_id, int seg_id, int next_seg_id){
	...
	int new_vehicle_index = gpu_data->lane_pool.vehicle_start_index[next_lane] +
			(gpu_data->lane_pool.first_veh_index[next_lane]+gpu_data->lane_pool.vehicle_counts[next_lane])%(gpu_data->lane_pool.ring_buffer_size[next_lane]);
	gpu_data->lane_vehicle_pool.vehicle_space[new_vehicle_index] = veh_id;
