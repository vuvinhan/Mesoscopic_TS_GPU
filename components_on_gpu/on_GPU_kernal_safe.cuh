...
__global__ void SupplySimulationVehiclePassingFirstOptimizeWarp(GPUMemory* gpu_data, int time_step, int node_length, GPUSharedParameter* data_setting_gpu, GPUVehicle *vpool_gpu);
...
__global__ void SupplySimulationVehiclePassingSynch(GPUMemory* gpu_data, int time_step, int node_length, GPUSharedParameter* data_setting_gpu, GPUVehicle *vpool_gpu);
...
__device__ int fastMod(int n, int m);
...
__global__ void SupplySimulationPreVehiclePassing(GPUMemory* gpu_data, int time_step, int segment_length, GPUSharedParameter* data_setting_gpu, GPUVehicle *vpool_gpu) {
 	...
 	if(time_step>0){
 		for(int lane_index = gpu_data->seg_pool.lane_start_index[seg_index]; lane_index<=gpu_data->seg_pool.lane_end_index[seg_index]; ++lane_index){
 			gpu_data->lane_pool.vehicle_counts[lane_index] = gpu_data->lane_pool.vehicle_counts[lane_index] - gpu_data->lane_pool.leaving_vehicle_counts[lane_index];
 			if(gpu_data->lane_pool.buffered_vehicle_counts[lane_index]>0){
 				...
 				gpu_data->lane_pool.first_veh_index[lane_index] = fastMod(gpu_data->lane_pool.first_veh_index[lane_index] + gpu_data->lane_pool.leaving_vehicle_counts[lane_index]
																								, RB_size);
 				gpu_data->lane_pool.buffered_vehicle_counts[lane_index] = gpu_data->lane_pool.buffered_vehicle_counts[lane_index] - can_pass_veh;
...
// 					gpu_data->lane_vehicle_pool.vehicle_space[new_vehicle_index] = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.buffered_first_veh_index[lane_index]];
//
// 					gpu_data->lane_pool.buffered_vehicle_counts[lane_index]--;
//
...
// 				}
 	...
 	for (int i = 0; i < can_insert_veh; i++) {
 		...
 		int index = gpu_data->lane_pool.vehicle_start_index[lane_index] + fastMod(gpu_data->lane_pool.first_veh_index[lane_index]+gpu_data->lane_pool.vehicle_counts[lane_index], RB_size);
 		gpu_data->lane_vehicle_pool.vehicle_space[index] = gpu_data->new_vehicles_every_time_step[time_step].new_vehicles[seg_index][i];
 	...
 	for(int lane_index = gpu_data->seg_pool.lane_start_index[seg_index]; lane_index<=gpu_data->seg_pool.lane_end_index[seg_index]; ++lane_index){
 		...
 		for (; queue_index < gpu_data->lane_pool.vehicle_counts[lane_index]; queue_index++) {
 			int vehicle_index = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[lane_index]+fastMod(gpu_data->lane_pool.first_veh_index[lane_index]+queue_index, RB_size)];
 		...
 		gpu_data->lane_pool.buffered_first_veh_index[lane_index] = fastMod(gpu_data->lane_pool.first_veh_index[lane_index]+gpu_data->lane_pool.vehicle_counts[lane_index], RB_size);
...
__device__ void initTimeDiffArray(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, int upstream_start_lane, int upstream_end_lane){
	for (int one_lane_index = upstream_start_lane; one_lane_index <= upstream_end_lane; one_lane_index++) {
		//if(gpu_data->lane_pool.timediff[one_lane_index]<0){
			...
			if (gpu_data->lane_pool.vehicle_counts[one_lane_index]>0 && gpu_data->seg_pool.output_capacity[seg_index]>0) {
				int first_vehicle_ID = gpu_data->lane_vehicle_pool.vehicle_space[gpu_data->lane_pool.vehicle_start_index[one_lane_index]+gpu_data->lane_pool.first_veh_index[one_lane_index]];
...
__device__ void updateTimeDiffArray(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, int lane_id, int seg_id){
	if (!gpu_data->lane_pool.blocked[lane_id]
			&& (gpu_data->lane_pool.vehicle_counts[lane_id]-gpu_data->lane_pool.leaving_vehicle_counts[lane_id]) > 0
			&& gpu_data->seg_pool.output_capacity[seg_id]>0)
	{
		int first_veh_index = gpu_data->lane_pool.vehicle_start_index[lane_id]+
				fastMod(gpu_data->lane_pool.first_veh_index[lane_id]+gpu_data->lane_pool.leaving_vehicle_counts[lane_id], gpu_data->lane_pool.ring_buffer_size[lane_id]);
		int first_vehicle_ID = gpu_data->lane_vehicle_pool.vehicle_space[first_veh_index];
...
__device__ void getFirstVehicleID(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, int* veh_id, int* lane_index, int upstream_start_lane, int upstream_end_lane){
	...
	*lane_index = -1;
	...
	if(*lane_index>=0){
		int first_veh_index = gpu_data->lane_pool.vehicle_start_index[*lane_index]+
				fastMod(gpu_data->lane_pool.first_veh_index[*lane_index]+gpu_data->lane_pool.leaving_vehicle_counts[*lane_index], gpu_data->lane_pool.ring_buffer_size[*lane_index]);
		*veh_id = gpu_data->lane_vehicle_pool.vehicle_space[first_veh_index];
	}
}
__device__ void updateAfterVehicleFinishingTrip(GPUMemory* gpu_data, GPUSharedParameter* data_setting_gpu, int lane_id, int seg_id){
	gpu_data->lane_pool.leaving_vehicle_counts[lane_id]++;
	//shift remaining vehicles to queue head
//	int start_vehicle_pool_index = gpu_data->lane_pool.vehicle_start_index[lane_id];
//	for (int j = 1; j <= (gpu_data->lane_pool.vehicle_counts[lane_id]-gpu_data->lane_pool.leaving_vehicle_counts[lane_id]); j++) {
//		gpu_data->lane_vehicle_pool.vehicle_space[start_vehicle_pool_index + j - 1] = gpu_data->lane_vehicle_pool.vehicle_space[start_vehicle_pool_index + j];
//	}
...
__device__ void updateAfterVehicleFinishingTripVP(GPUMemory* gpu_data, GPUSharedParameter* data_setting_gpu, int lane_id, int seg_id){
	gpu_data->lane_pool.leaving_vehicle_counts[lane_id]++;
//	int start_vehicle_pool_index = gpu_data->lane_pool.vehicle_start_index[lane_id];
//	for (int j = 1; j <= (gpu_data->lane_pool.vehicle_counts[lane_id]-gpu_data->lane_pool.leaving_vehicle_counts[lane_id]); j++) {
//		gpu_data->lane_vehicle_pool.vehicle_space[start_vehicle_pool_index + j - 1] = gpu_data->lane_vehicle_pool.vehicle_space[start_vehicle_pool_index + j];
...
__device__ void updateAfterVehicleMovingToBuffer(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, GPUSharedParameter* data_setting_gpu, int time_step, int veh_id, int lane_id, int seg_id, int next_seg_id){
	...
	int new_vehicle_index = gpu_data->lane_pool.vehicle_start_index[next_lane]
							+fastMod(gpu_data->lane_pool.buffered_first_veh_index[next_lane] + gpu_data->lane_pool.buffered_vehicle_counts[next_lane], gpu_data->lane_pool.ring_buffer_size[next_lane]);
...
__device__ void updateAfterVehicleMovingToBufferVP(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, GPUSharedParameter* data_setting_gpu, int time_step, int veh_id, int lane_id, int seg_id, int next_seg_id){
	vpool_gpu[veh_id].next_path_index++;
...
__device__ void updateAfterVehicleMovingToNextSeg(GPUMemory* gpu_data, GPUVehicle *vpool_gpu, GPUSharedParameter* data_setting_gpu, int time_step, int veh_id, int lane_id, int seg_id, int next_seg_id){
	...
	gpu_data->lane_pool.buffered_vehicle_counts[next_lane]++;
	gpu_data->seg_pool.input_capacity[next_seg_id]--;
	atomicSub(&gpu_data->seg_pool.empty_space[seg_id],1);
...
//		return;
//
//	//if(gpu_data->lane_pool.vehicle_passed_to_the_lane_counts[lane_index]==0 && gpu_data->lane_pool.leaving_vehicle_counts[lane_index]==0 && gpu_data->lane_pool.buffered_vehicle_counts[lane_index]==0)
//	//	return;
//
...
__device__ int Arrayin[20];
__device__ int Arrayout[20];
__device__ void __gpu_sync2(int goalVal)
{
